...
def read_weight(initializer: onnx.TensorProto):
    if not initializer.raw_data:
        print(f"NO raw data available for {initializer.name}")
        return
    shape = initializer.dims
    data  = np.frombuffer(initializer.raw_data, dtype=np.float64).reshape(shape)
    print("\n**************parse weight data******************")
...
def main(): 
    model = onnx.load("models/convnet-sample.onnx")
