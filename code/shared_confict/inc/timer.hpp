...
void Timer::duration_cpu(std::string msg){
    ...
    if(std::is_name<span, s>::value) {str = "s";}
    else if(std::is_name<span, ms>::value) {str = "ms";}
    else if(std::is_name<span, us>::value) {str = "us";}
    else if(std::is_name<span, ns>::value) {str = "ns";}
